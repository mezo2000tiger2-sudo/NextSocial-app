...
export function PostItem({ post, user, page }: { post: Post; user: any, page: string }) {
  ...
  const [saved, setsaved] = useState(() => post.bookmarked == true);
  ...
  const { mutate, isPending: isLiking } = useMutation({
    mutationKey: ['like', post._id],
    mutationFn: () => likeOrUnlickApi(post._id),
    onSuccess: () => {
  ...
  const { data, mutate: postDelete } = useMutation({
    mutationKey: ['delete', post._id],
    mutationFn: () => deletPost(post._id),
    onSuccess: () => {
      querClient.invalidateQueries({
        queryKey: ['get posts', page],
        refetchType: 'all'
  ...
  const { data: commentDataDelet, mutate: commentDelete } = useMutation({
    mutationKey: ['comment delete', post._id],
  ...
  const { mutate: saveUnsave } = useMutation({
    mutationKey: ['save', post._id],
    mutationFn: () => saveOrUnsaveApi(post._id),
    onSuccess: () => {
      querClient.invalidateQueries({
        queryKey: ['get posts', page],
        refetchType: 'all'
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 max-w-2xl mx-auto overflow-hidden w-full">
          ...
          <DropdownMenuContent>
            <DropdownMenuGroup>
              ...
              {user?.user._id == post.user._id && <>
                <DropdownMenuItem onSelect={(e) => { setEditOpen(true) }}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                  </svg>
                  Edit
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => postDelete()} className="text-red-600 hover:bg-red-100 focus:bg-red-100 transition-colors">
        ...
        <div className="flex items-center justify-between px-2 pt-2">
          ...
          <div className="flex items-start gap-3 cursor-pointer group" onClick={() => openProfile(post.sharedPost?.user._id)}>
            ...
            <div>
              ...
              <div className="flex items-center gap-1 text-slate-500 text-xs">
                {post.body == 'updated profile picture.' ? '' : <><span>@{post.user.username}</span></>}
              </div>


            </div>
          </div>
          <Link href={`singlepost/${post.sharedPost.id}`} className="text-xs font-semibold text-blue-600 hover:bg-blue-200 p-1.5 transition-colors rounded-lg flex justify-center items-center gap-1"> Original post <ExternalLink className="size-3" /></Link>
        </div>
        <p className=" mt-2 text-left pb-3 text-slate-800 text-sm px-2">
          {post.sharedPost.body == 'updated profile picture.' ? '' : post.sharedPost.body}
        </p>
        <div className="w-full bg-gradient-to-br from-teal-400 via-slate-500 to-purple-400 flex items-center justify-center overflow-hidden">
          <img src={post.sharedPost.image} className="w-full max-h-120 object-cover" alt="" />
        </div>
      </div>
      </>}

      {/* Likes / shares / comments count */}
      <div className="hidden md:flex items-center justify-between px-0.5 sm:px-4 py-2 text-slate-500 text-sm border-b border-slate-100">
        <div className="flex items-center gap-1">
          <div className="w-5 h-5 rounded-full bg-blue-500 hidden md:flex items-center justify-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href={`singlepost/${post._id}`} className="text-blue-500 font-semibold hover:underline">View details</Link>
        </div>
      </div>
      ...
      <div className="flex items-center border-b border-slate-100">
        ...
        >
          ...
          Like
          <span className="md:hidden text-xs font-semibold tabular-nums">{likes}</span>
        </button>
        <button className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 0 1-.923 1.785A5.969 5.969 0 0 0 6 21c1.282 0 2.47-.402 3.445-1.087.81.22 1.668.337 2.555.337Z" />
          </svg>
          Comment
          <span className="md:hidden text-xs font-semibold tabular-nums">{post.commentsCount}</span>
        </button>
        ...
        <Dialog open={shareOpen} onOpenChange={setShareOpen}>
          <DialogTrigger asChild>
            <button onClick={() => setShareOpen(true)} className="flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
              ...
              Share
              <span className="md:hidden text-xs font-semibold tabular-nums">{post.sharesCount}</span>
            </button>
          </DialogTrigger>
      ...
      {post.topComment != null && (
        <>
          ...
          <div className="mx-4 my-3 bg-slate-50 rounded-xl p-3 ">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">Top Comment</p>
            <div className="flex justify-between items-start w-full">

              <div className="flex items-start gap-3  w-2/3">
                <Image src={post.topComment.image || logo} alt="topcommentlogo" width={36} height={36} className="w-9 h-9 rounded-full" />
                <div className="bg-white rounded-lg p-3 w-full ">
                  <p className="font-bold text-slate-900 text-sm">{post.topComment.commentCreator.name}</p>
                  <p className="text-slate-600 text-sm mt-0.5">{post.topComment.content}</p>
                  {post.topComment.image && <img src={post.topComment.image} alt="" className=" w-full h-50 rounded-lg object-cover" />}
                </div>
              </div>
              {user?.user._id == post.topComment.commentCreator._id && <>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="border-0 ring-0 cursor-pointer focus-visible:outline-0">
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuGroup>
                      ...
                      <DropdownMenuItem onSelect={(e) => e.preventDefault()} onClick={() => commentDelete({ postId: post._id, commentId: post.topComment?._id! })} className="text-red-600 hover:bg-red-100 focus:bg-red-100 transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4 text-red-600"><path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg>
                        Delete
                      </DropdownMenuItem>
